...
func (u *BlockRepository) TxUpdateAndDeleteBlocks(user *entity.User, blocks *entity.Blocks) error {
	...
	err := u.blockDbHandler.Transaction(func() error {
		...
		if err := u.blockDbHandler.FindAllByUserID(&registedBlocks, strconv.FormatUint(uint64(user.GetID()), 10)); err != nil {
		...
		/*
			...
			for _, registedBlockEntity := range registedBlockEntities {
				...
				idx := blocks.FindByTargetTwitterID(registedBlockEntity.GetTargetTwitterID())

				// blocksの長さが0なら登録されていない。
				// または、idxがblocks総数以上の場合は登録されていない
				if len(*blocks) == 0 || len(*blocks) <= idx {
					u.loggerHandler.Debugf("Target(target_twitter_id=%s,flag=%d) is converted entry", registedBlockEntity.GetTargetTwitterID(), registedBlockEntity.GetFlag())
					IDs = append(IDs, registedBlockEntity.GetID())
				}
			}
		*/
		...
		/*
			if len(registedBlockEntities) > 0 {
				for n, block := range *blocks {
					idx := registedBlockEntities.FindByTargetTwitterID(block.GetTargetTwitterID())
					if len(registedBlockEntities) < idx && registedBlockEntities[idx].GetTargetTwitterID() == block.GetTargetTwitterID() {
						if registedBlockEntities[idx].GetFlag() == 1 { // TODO: 定数化
							(*blocks)[n].Converted()
						} else {
							(*blocks)[n].NotConverted()
						}
						u.loggerHandler.Debugf("Target(target_twitter_id=%s,flag=%d) won`t be updated", (*blocks)[n].GetTargetTwitterID(), (*blocks)[n].GetFlag())
					}
				}
			}*/
		blocks = blocks.GetNotConvertedBlocks()
		if len(*blocks) > 0 {
			if err := u.blockDbHandler.CreateNewBlocks(blocks.ToBlockEntities(), "user_id", "twitter_id"); err != nil {
